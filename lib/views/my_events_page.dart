...
import 'package:hedieaty/models/local/local_event_model.dart';
...
class _MyEventsPageState extends State<MyEventsPage> {
  ...
  Future<List<RemoteEventModel>> _fetchEvents() async {
    return _firestoreService.getUserEvents(sortBy)
  ...
  Future<void> _addEvent(RemoteEventModel event) async {

    await dbHelper.insertOrUpdateEvent({
      'id': event.id,
      'name': event.name,
      ...
      'status': event.status,
      'isSynced': 0,
      'userId': _authService.currentUserUid
    });
  ...
  Future<void> _editEvent(String eventId, RemoteEventModel event) async {
    try{
      await dbHelper.insertOrUpdateEvent({
        'id': eventId,
        'name': event.name,
      ...
      if (connectivity != ConnectivityResult.none) {
        ...
        await dbHelper.markEventAsSynced(eventId);
        print('Event stored remotely and synced');
      }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Event List
              Expanded(
                child: FutureBuilder<List<RemoteEventModel>>(
                  future: _fetchEvents(),
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Icon(Icons.calendar_month, color: Colors.teal, size: 16),
                SizedBox(width: 4),
                Text(
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => MyGiftsPage(
                userId: _authService.currentUserUid ?? 'none',
                eventId: event.id,
  ...
  Widget _buildAddEventButton() {
    return FloatingActionButton(
      key: const ValueKey('add_event_button'),
  ...
  void _showAddEventDialog() {
    showDialog(
      ...
      builder: (context) => EventDialog(
        ...
        determineStatus: _determineEventStatus,
      ),
    );
  }

  void _showEditEventDialog(RemoteEventModel event) {
    showDialog(
      context: context,
      builder: (context) => EventDialog(
        event: event,
        onSave: (updatedEvent) => _editEvent(event.id!, updatedEvent),
        determineStatus: _determineEventStatus,
      ),
    );
  }

  void _showDeleteConfirmation(RemoteEventModel event) {
    showDialog(
      context: context,
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            TextField(
              key: const ValueKey('event_name_field'),
            ...
            Row(
              children: [
                ...
                TextButton(
                  key: const ValueKey('choose_date_button'),
                  onPressed: _pickDate,
      ...
      actions: [
        TextButton(
          key: const ValueKey('cancel_event_button'),
          onPressed: () => Navigator.of(context).pop(),
          child: Text('Cancel', style: TextStyle(color: Colors.teal)),
        ),
        TextButton(
